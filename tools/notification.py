...
class NotificationTool(BaseTool):
    ...
    description: str = (
        "向运维人员发送通知消息，支持钉钉、Slack、邮件等多渠道。"
        "可用于故障告警、处理进展通知、故障恢复通知等场景。"
        "输入参数: message（消息内容）, title（标题）, "
        "severity（严重程度: critical/warning/info/recovery）"
    )
    ...
    def _run(
        ...
        for channel in active_channels:
            try:
                if channel == "dingtalk":
                    results["dingtalk"] = self._send_dingtalk(full_title, message, severity)
                elif channel == "slack":
                    results["slack"] = self._send_slack(full_title, message, severity)
                elif channel == "email":
                    email_list = recipients or settings.email_recipients_list
                    results["email"] = self._send_email(full_title, message, email_list)
    ...
    def _send_dingtalk(self, title: str, message: str, severity: str) -> dict:
        """发送钉钉群消息"""
        payload = {
            "msgtype": "markdown",
            "markdown": {
    ...
    def _send_slack(self, title: str, message: str, severity: str) -> dict:
        """发送 Slack 消息"""
        color_map = {
            "critical": "#FF0000",
            "warning": "#FFA500",
    ...
    def _send_email(self, title: str, message: str, recipients: List[str]) -> dict:
        """发送邮件通知"""
        if not recipients:
