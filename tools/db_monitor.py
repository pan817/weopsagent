...
class DBMonitorTool(BaseTool):
    ...
    连接 MySQL/PostgreSQL 数据库，收集以下信息：
    - 数据库连接是否正常
    - 当前活跃连接数
    - 慢查询日志（最近 N 条）
    - 锁等待情况
    - 表空间使用情况
    """
    name: str = "monitor_database"
    description: str = (
        "监控数据库运行状态，包括连接池状态、慢查询、锁等待等。"
        "可以快速发现数据库性能瓶颈和连接问题。"
        "输入参数: host（DB 地址，可选）, slow_query_threshold_ms（慢查询阈值毫秒，默认1000）, "
        "top_n_slow_queries（返回慢查询条数，默认10）"
    )
    args_schema: Type[BaseModel] = DBMonitorInput
