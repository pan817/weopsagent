...
def _get_agent(console_confirm_mode: bool = True) -> Any:
    """获取 Recovery Subagent 编译实例（按确认模式分别缓存）"""
...
def run_recovery(task_description: str) -> str:
    ...
    try:
        agent = _get_agent(_console_confirm_mode)
        result = agent.invoke(
            {"messages": [HumanMessage(content=task_description)]},
            config=RunnableConfig(
                configurable={"thread_id": "recovery-task"},
            ),
        )
